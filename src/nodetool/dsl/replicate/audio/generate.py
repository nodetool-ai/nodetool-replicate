...
class MusicGen(GraphNode):
    ...
    model_version: nodetool.nodes.replicate.audio.generate.MusicGen.Model_version = Field(default=nodetool.nodes.replicate.audio.generate.MusicGen.Model_version.STEREO_MELODY_LARGE, description='Model to use for generation')
...
class RealisticVoiceCloning(GraphNode):
    ...
    song_input: types.AudioRef | GraphNode | tuple[GraphNode, str] = Field(default=types.AudioRef(type='audio', uri='', asset_id=None, data=None), description='Upload your audio file here.')
    ...
    pitch_change: nodetool.nodes.replicate.audio.generate.RealisticVoiceCloning.Pitch_change = Field(default=nodetool.nodes.replicate.audio.generate.RealisticVoiceCloning.Pitch_change.NO_CHANGE, description='Adjust pitch of AI vocals. Options: `no-change`, `male-to-female`, `female-to-male`.')
...
class StyleTTS2(GraphNode):
    ...
    reference: types.AudioRef | GraphNode | tuple[GraphNode, str] = Field(default=types.AudioRef(type='audio', uri='', asset_id=None, data=None), description='Reference speech to copy style from')
