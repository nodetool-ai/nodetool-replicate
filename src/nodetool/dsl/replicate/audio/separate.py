...
class Demucs(GraphNode):
    ...
    stem: nodetool.nodes.replicate.audio.separate.Demucs.Stem = Field(default=nodetool.nodes.replicate.audio.separate.Demucs.Stem.NONE, description='If you just want to isolate one stem, you can choose it here.')
    audio: types.AudioRef | GraphNode | tuple[GraphNode, str] = Field(default=types.AudioRef(type='audio', uri='', asset_id=None, data=None), description='Upload the file to be processed here.')
    model: nodetool.nodes.replicate.audio.separate.Demucs.Model = Field(default=nodetool.nodes.replicate.audio.separate.Demucs.Model.HTDEMUCS, description='Choose the demucs audio that proccesses your audio. The readme has more information on what to choose.')
    ...
    clip_mode: nodetool.nodes.replicate.audio.separate.Demucs.Clip_mode = Field(default=nodetool.nodes.replicate.audio.separate.Demucs.Clip_mode.RESCALE, description='Choose the strategy for avoiding clipping. Rescale will rescale entire signal if necessary or clamp will allow hard clipping.')
    mp3_preset: nodetool.nodes.replicate.audio.separate.Demucs.Mp3_preset = Field(default=nodetool.nodes.replicate.audio.separate.Demucs.Mp3_preset._2, description='Choose the preset for the MP3 output. Higher is faster but worse quality. If MP3 is not selected as the output type, this has no effect.')
    wav_format: nodetool.nodes.replicate.audio.separate.Demucs.Wav_format = Field(default=nodetool.nodes.replicate.audio.separate.Demucs.Wav_format.INT24, description='Choose format for the WAV output. If WAV is not selected as the output type, this has no effect.')
    ...
    output_format: nodetool.nodes.replicate.audio.separate.Demucs.Output_format = Field(default=nodetool.nodes.replicate.audio.separate.Demucs.Output_format.MP3, description='Choose the audio format you would like the result to be returned in.')
