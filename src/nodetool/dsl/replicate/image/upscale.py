...
class ClarityUpscaler(GraphNode):
    ...
    sd_model: nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Sd_model = Field(
        default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Sd_model.JUGGERNAUT_REBORN_SAFETENSORS__338B85BC4F,
    ...
    scheduler: nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Scheduler = Field(
        default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Scheduler.DPM___3M_SDE_KARRAS,
        description="scheduler",
    )
    creativity: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.35, description="Creativity, try from 0.3 - 0.9"
    )
    lora_links: str | GraphNode | tuple[GraphNode, str] = Field(
        default="",
        description="Link to a lora file you want to use in your upscaling. Multiple links possible, seperated by comma",
    )
    downscaling: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=False,
    ...
    resemblance: float | GraphNode | tuple[GraphNode, str] = Field(
    ...
    tiling_width: (
        ...
        default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Tiling_width._112,
        description="Fractality, set lower tile width for a high Fractality",
    )
    output_format: (
        nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Output_format
    ) = Field(
        default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Output_format.PNG,
        description="Format of the output images",
    )
    tiling_height: (
        nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Tiling_height
    ) = Field(
        default=nodetool.nodes.replicate.image.upscale.ClarityUpscaler.Tiling_height._144,
        description="Fractality, set lower tile height for a high Fractality",
    )
    custom_sd_model: str | GraphNode | tuple[GraphNode, str] = Field(
    ...
    downscaling_resolution: int | GraphNode | tuple[GraphNode, str] = Field(
        default=768, description="Downscaling resolution"
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.ClarityUpscaler"
...
class GFPGAN(GraphNode):
    ...
    img: types.ImageRef | GraphNode | tuple[GraphNode, str] = Field(
        default=types.ImageRef(type="image", uri="", asset_id=None, data=None),
        description="Input",
    )
    ...
    version: nodetool.nodes.replicate.image.upscale.GFPGAN.Version = Field(
        default=nodetool.nodes.replicate.image.upscale.GFPGAN.Version.V1_4,
        description="GFPGAN version. v1.3: better quality. v1.4: more details and better identity.",
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.GFPGAN"
...
class HighResolutionControlNetTile(GraphNode):
    ...
    hdr: float | GraphNode | tuple[GraphNode, str] = Field(
    ...
    steps: int | GraphNode | tuple[GraphNode, str] = Field(
        default=8, description="Steps"
    )
    format: (
        nodetool.nodes.replicate.image.upscale.HighResolutionControlNetTile.Format
    ) = Field(
        default=nodetool.nodes.replicate.image.upscale.HighResolutionControlNetTile.Format.JPG,
    ...
    scheduler: (
        ...
        default=nodetool.nodes.replicate.image.upscale.HighResolutionControlNetTile.Scheduler.DDIM,
    ...
    guess_mode: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=False,
    ...
    resolution: (
        ...
        default=nodetool.nodes.replicate.image.upscale.HighResolutionControlNetTile.Resolution._2560,
    ...
    lora_details_strength: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1, description="Strength of the image's details"
    )
    lora_sharpness_strength: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1.25,
    ...
    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.HighResolutionControlNetTile"
...
class MagicImageRefiner(GraphNode):
    ...
    scheduler: nodetool.nodes.replicate.image.upscale.MagicImageRefiner.Scheduler = (
        Field(
            default=nodetool.nodes.replicate.image.upscale.MagicImageRefiner.Scheduler.DDIM,
            description="Choose a scheduler.",
        )
    )
    creativity: float | GraphNode | tuple[GraphNode, str] = Field(
    ...
    resolution: nodetool.nodes.replicate.image.upscale.MagicImageRefiner.Resolution = (
        Field(
            default=nodetool.nodes.replicate.image.upscale.MagicImageRefiner.Resolution.ORIGINAL,
            description="Image resolution",
        )
    )
    resemblance: float | GraphNode | tuple[GraphNode, str] = Field(
    ...
    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.MagicImageRefiner"
...
class RealEsrGan(GraphNode):
    ...
    face_enhance: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=False, description="Run GFPGAN face enhancement along with upscaling"
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.RealEsrGan"
...
class Swin2SR(GraphNode):
    ...
    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.Swin2SR"
...
class SwinIR(GraphNode):
    ...
    jpeg: int | GraphNode | tuple[GraphNode, str] = Field(
        default=40,
    ...
    noise: nodetool.nodes.replicate.image.upscale.SwinIR.Noise = Field(
        default=nodetool.nodes.replicate.image.upscale.SwinIR.Noise._15,
    ...
    task_type: nodetool.nodes.replicate.image.upscale.SwinIR.Task_type = Field(
        default=nodetool.nodes.replicate.image.upscale.SwinIR.Task_type.REAL_WORLD_IMAGE_SUPER_RESOLUTION_LARGE,
        description="Choose a task",
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.SwinIR"
...
class UltimateSDUpscale(GraphNode):
    ...
    upscaler: nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Upscaler = Field(
        default=nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Upscaler._4X_ULTRASHARP,
        description="Upscaler",
    )
    mask_blur: int | GraphNode | tuple[GraphNode, str] = Field(
        default=8, description="Mask Blur"
    )
    mode_type: nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Mode_type = (
        Field(
            default=nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Mode_type.LINEAR,
            description="Mode Type",
        )
    )
    scheduler: nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Scheduler = (
        Field(
            default=nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Scheduler.NORMAL,
            description="Scheduler",
        )
    )
    tile_width: int | GraphNode | tuple[GraphNode, str] = Field(
        default=512, description="Tile Width"
    )
    upscale_by: float | GraphNode | tuple[GraphNode, str] = Field(
        default=2, description="Upscale By"
    )
    tile_height: int | GraphNode | tuple[GraphNode, str] = Field(
        default=512, description="Tile Height"
    )
    sampler_name: (
        nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Sampler_name
    ) = Field(
        default=nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Sampler_name.EULER,
        description="Sampler",
    )
    tile_padding: int | GraphNode | tuple[GraphNode, str] = Field(
        default=32, description="Tile Padding"
    )
    seam_fix_mode: (
        nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Seam_fix_mode
    ) = Field(
        default=nodetool.nodes.replicate.image.upscale.UltimateSDUpscale.Seam_fix_mode.NONE,
        description="Seam Fix Mode",
    )
    seam_fix_width: int | GraphNode | tuple[GraphNode, str] = Field(
        default=64, description="Seam Fix Width"
    )
    negative_prompt: str | GraphNode | tuple[GraphNode, str] = Field(
        default="", description="Negative Prompt"
    )
    positive_prompt: str | GraphNode | tuple[GraphNode, str] = Field(
        default="Hey! Have a nice day :D", description="Positive Prompt"
    )
    seam_fix_denoise: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1, description="Seam Fix Denoise"
    )
    seam_fix_padding: int | GraphNode | tuple[GraphNode, str] = Field(
        default=16, description="Seam Fix Padding"
    )
    seam_fix_mask_blur: int | GraphNode | tuple[GraphNode, str] = Field(
    ...
    force_uniform_tiles: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=True, description="Force Uniform Tiles"
    )
    use_controlnet_tile: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=True, description="Use ControlNet Tile"
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.UltimateSDUpscale"
...
class ruDallE_SR(GraphNode):
    ...
    scale: nodetool.nodes.replicate.image.upscale.ruDallE_SR.Scale = Field(
        default=nodetool.nodes.replicate.image.upscale.ruDallE_SR.Scale._4,
        description="Choose up-scaling factor",
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.upscale.ruDallE_SR"
