...
class Nano_Banana_Pro(SingleOutputGraphNode[types.ImageRef], GraphNode[types.ImageRef]):
    """
    Google's state of the art image generation and editing model 🍌🍌
    """

    Resolution: typing.ClassVar[type] = (
        nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Resolution
    )
    Aspect_ratio: typing.ClassVar[type] = (
        nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Aspect_ratio
    )
    Output_format: typing.ClassVar[type] = (
        nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Output_format
    )
    Safety_filter_level: typing.ClassVar[type] = (
        nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Safety_filter_level
    )

    prompt: str | OutputHandle[str] | None = connect_field(
        default=None, description="A text description of the image you want to generate"
    )
    resolution: nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Resolution = (
        Field(
            default=nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Resolution(
                "2K"
            ),
    ...
    aspect_ratio: (
        ...
        default=nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Aspect_ratio(
            "match_input_image"
        ),
        description="Aspect ratio of the generated image",
    )
    output_format: (
        ...
        default=nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Output_format(
            "jpg"
        ),
        description="Format of the output image",
    )
    safety_filter_level: (
        ...
        default=nodetool.nodes.replicate.image.generate.Nano_Banana_Pro.Safety_filter_level(
            "block_only_high"
        ),
        description="block_low_and_above is strictest, block_medium_and_above blocks some prompts, block_only_high is most permissive but some prompts will still be blocked",
    )
    ...
    @classmethod
    def get_node_class(cls) -> type[BaseNode]:
        return nodetool.nodes.replicate.image.generate.Nano_Banana_Pro
...
class Z_Image_Turbo(SingleOutputGraphNode[types.ImageRef], GraphNode[types.ImageRef]):
    """
    Z-Image Turbo is a super fast text-to-image model of 6B parameters developed by Tongyi-MAI.
    """

    Output_format: typing.ClassVar[type] = (
        nodetool.nodes.replicate.image.generate.Z_Image_Turbo.Output_format
    )
    ...
    go_fast: bool | OutputHandle[bool] = connect_field(
        default=True, description="Apply additional optimizations for faster generation"
    )
    output_format: (
        nodetool.nodes.replicate.image.generate.Z_Image_Turbo.Output_format
    ) = Field(
        default=nodetool.nodes.replicate.image.generate.Z_Image_Turbo.Output_format(
            "jpg"
        ),
        description="Format of the output images",
    )
    guidance_scale: float | OutputHandle[float] = connect_field(
        default=0, description="Guidance scale. Should be 0 for Turbo models"
    )
    output_quality: int | OutputHandle[int] = connect_field(
        default=80,
        description="Quality when saving the output images, from 0 to 100. 100 is best quality, 0 is lowest quality. Not relevant for .png outputs",
    ...
    @classmethod
    def get_node_class(cls) -> type[BaseNode]:
        return nodetool.nodes.replicate.image.generate.Z_Image_Turbo
