...
class CodeFormer(GraphNode):
    ...
    upscale: int | GraphNode | tuple[GraphNode, str] = Field(
        default=2, description="The final upsampling scale of the image"
    )
    face_upsample: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=True,
        description="Upsample restored faces for high-resolution AI-created images",
    )
    background_enhance: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=True, description="Enhance background image with Real-ESRGAN"
    )
    codeformer_fidelity: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.5,
    ...
    def get_node_type(cls):
        return "replicate.image.enhance.CodeFormer"
...
class Maxim(GraphNode):
    ...
    @classmethod
    def get_node_type(cls):
        return "replicate.image.enhance.Maxim"
...
class Night_Enhancement(GraphNode):
    ...
    @classmethod
    def get_node_type(cls):
        return "replicate.image.enhance.Night_Enhancement"


class OldPhotosRestoration(GraphNode):
    """Bringing Old Photos Back to Life"""

    HR: bool | GraphNode | tuple[GraphNode, str] = Field(
    ...
    with_scratch: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=False, description="whether the input image is scratched"
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.enhance.OldPhotosRestoration"
...
class Supir_V0F(GraphNode):
    ...
    min_size: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1024, description="Minimum resolution of output images."
    )
    n_prompt: str | GraphNode | tuple[GraphNode, str] = Field(
    ...
    s_stage1: int | GraphNode | tuple[GraphNode, str] = Field(
        default=-1, description="Control Strength of Stage1 (negative means invalid)."
    )
    s_stage2: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1, description="Control Strength of Stage2."
    )
    edm_steps: int | GraphNode | tuple[GraphNode, str] = Field(
        default=50, description="Number of steps for EDM Sampling Schedule."
    )
    linear_CFG: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=False,
    ...
    color_fix_type: nodetool.nodes.replicate.image.enhance.Supir_V0F.Color_fix_type = (
        Field(
            default=nodetool.nodes.replicate.image.enhance.Supir_V0F.Color_fix_type.WAVELET,
            description="Color Fixing Type..",
        )
    )
    spt_linear_CFG: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1, description="Start point of linearly increasing CFG."
    )
    linear_s_stage2: bool | GraphNode | tuple[GraphNode, str] = Field(
        default=False,
        description="Linearly (with sigma) increase s_stage2 from 'spt_linear_s_stage2' to s_stage2.",
    )
    spt_linear_s_stage2: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0, description="Start point of linearly increasing s_stage2."
    )

    @classmethod
    def get_node_type(cls):
        return "replicate.image.enhance.Supir_V0F"
...
class Supir_V0Q(GraphNode):
    ...
    color_fix_type: nodetool.nodes.replicate.image.enhance.Supir_V0Q.Color_fix_type = (
        Field(
            default=nodetool.nodes.replicate.image.enhance.Supir_V0Q.Color_fix_type.WAVELET,
            description="Color Fixing Type..",
        )
    )
    spt_linear_CFG: float | GraphNode | tuple[GraphNode, str] = Field(
    ...
    @classmethod
    def get_node_type(cls):
        return "replicate.image.enhance.Supir_V0Q"
