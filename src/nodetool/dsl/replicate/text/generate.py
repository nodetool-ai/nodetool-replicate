...
class Claude_3_7_Sonnet(GraphNode):
    """The most intelligent Claude model and the first hybrid reasoning model on the market (claude-3-7-sonnet-20250219)"""

    image: str | None | GraphNode | tuple[GraphNode, str] = Field(default=None, description='Optional input image. Images are priced as (width px * height px)/750 input tokens')
    prompt: str | None | GraphNode | tuple[GraphNode, str] = Field(default=None, description='Input prompt')
    max_tokens: int | GraphNode | tuple[GraphNode, str] = Field(default=8192, description='Maximum number of output tokens')
    system_prompt: str | GraphNode | tuple[GraphNode, str] = Field(default='', description='System prompt')
    max_image_resolution: float | GraphNode | tuple[GraphNode, str] = Field(default=0.5, description='Maximum image resolution in megapixels. Scales down image before sending it to Claude, to save time and money.')

    @classmethod
    def get_node_type(cls): return "replicate.text.generate.Claude_3_7_Sonnet"



class Deepseek_R1(GraphNode):
    """A reasoning model trained with reinforcement learning, on par with OpenAI o1"""

    top_p: float | GraphNode | tuple[GraphNode, str] = Field(default=1, description='Top-p (nucleus) sampling')
    prompt: str | GraphNode | tuple[GraphNode, str] = Field(default='', description='Prompt')
    max_tokens: int | GraphNode | tuple[GraphNode, str] = Field(default=20480, description='The maximum number of tokens the model should generate as output.')
    temperature: float | GraphNode | tuple[GraphNode, str] = Field(default=0.1, description='The value used to modulate the next token probabilities.')
    ...
    @classmethod
    def get_node_type(cls): return "replicate.text.generate.Deepseek_R1"
